...
export default function ScrollPresentation({
	...
	const [expandedInsight, setExpandedInsight] = useState<InsightData | null>(
		null,
	);
	...
	const switchMode = (next: ViewMode) => {
		...
		const outEl =
			viewMode === "presentation" ? presentationRef.current : gridRef2.current;
		const inEl =
			next === "presentation" ? presentationRef.current : gridRef2.current;
		if (!outEl || !inEl) {
			setViewMode(next);
		...
		gsap.set(inEl, {
			visibility: "visible",
			opacity: 0,
			y: next === "presentation" ? 60 : -60,
			scale: 0.96,
			pointerEvents: "none",
		});
		if (next === "grid") {
			gridCardRefs.current.forEach((el) => {
				if (el) gsap.set(el, { opacity: 0, x: 0, y: 0, scale: 1 });
			});
			const headerCard = gridRef2.current?.querySelector(
				"[data-header-card]",
			) as HTMLElement | null;
			if (headerCard)
				gsap.set(headerCard, { opacity: 0, x: 0, y: 0, scale: 1 });
		}
		...
		<div
			...
			<div
				...
				style={{
					...
					transform:
						viewMode === "presentation" || cornerHovered
							? "translateY(0)"
							: "translateY(8px)",
					transition: "opacity 0.2s ease, transform 0.2s ease",
					pointerEvents:
						viewMode === "presentation" || cornerHovered ? "auto" : "none",
				}}>
				...
				<button
					onClick={() =>
						switchMode(viewMode === "presentation" ? "grid" : "presentation")
					}
					style={{
						padding: "8px 14px",
						background: "var(--surface, #1a1f2e)",
					...
					{Array.from({ length: totalSlides }).map((_, i) => (
						<button
							...
							style={{
								...
								border: "none",
								background:
									currentSlide === i ? "var(--primary)" : "var(--border-color)",
								cursor: "pointer",
...
function HeaderContent({
	...
	return (
		<div style={{ textAlign: "center", maxWidth: 700 }}>
			...
			<p style={{ fontSize: 15, color: "var(--text-tertiary)", marginTop: 16 }}>
...
function InsightContent({
	...
	const auroraData = insight.chartOptions
		? (() => {
				...
				if (opts._auroraData)
					return opts._auroraData as {
						labels: string[];
						datasets: { label?: string; data: number[] }[];
					};
				const xAxis = opts.xAxis as { data?: string[] } | undefined;
				const series = opts.series as
					| { type?: string; data?: unknown[] }[]
					| undefined;
				if (xAxis?.data && series?.[0]?.data) {
					const labels = xAxis.data;
					const datasets = (
						series as { type?: string; data?: unknown[]; name?: string }[]
					).map((s) => ({
						label: s.name ?? "",
						data: (s.data as ({ value?: number } | number)[]).map((d) =>
							typeof d === "number"
								? d
				...
				const pieSeries = series?.[0] as
					| { type?: string; data?: { name: string; value: number }[] }
					| undefined;
				if (
					pieSeries?.data &&
					Array.isArray(pieSeries.data) &&
					pieSeries.data[0]?.name !== undefined
				) {
					return {
						labels: pieSeries.data.map((d: { name: string }) => d.name),
						datasets: [
							{ data: pieSeries.data.map((d: { value: number }) => d.value) },
						],
					};
				}
				return null;
	...
	const chartType = insight.chartOptions
		? (() => {
				...
				return (supported.includes(t) ? t : "bar") as
					| "bar"
					| "line"
					| "area"
					| "pie"
...
function renderChart(
	...
	const opts = insight.chartOptions as Record<string, unknown>;
	const auroraData = opts._auroraData as
			...
			<div
				...
				{auroraData.labels.map((label, i) => {
					const value = Math.min(
						100,
						Math.max(0, auroraData.datasets?.[0]?.data?.[i] ?? 0),
					);
							...
							<div style={{ display: "flex", justifyContent: "space-between" }}>
								<span
									style={{
										fontSize: "0.82rem",
										fontWeight: 500,
										color: "var(--text-secondary, rgba(230,236,244,0.8))",
									}}>
									{label}
								</span>
								<span
									style={{
										fontSize: "0.78rem",
										color: "var(--text-tertiary, rgba(170,185,210,0.6))",
									}}>
									{value}%
								</span>
							</div>
		...
		preview && auroraData
			? chartType === "heatmap"
				? {
						...
						datasets: auroraData.datasets
							.slice(0, 4)
							.map((ds) => ({ ...ds, data: ds.data.slice(0, 6) })),
					}
				: chartType === "radar"
					? {
							labels: auroraData.labels.slice(0, 4),
							datasets: auroraData.datasets.map((ds) => ({
								...ds,
...
const WIDGET_H: Record<string, number> = {
	...
	candlestick: 400,
	"bar-race": 400,
	"hierarchical-bar": 400,
	"diverging-bar": 400,
	"radial-stacked-bar": 400,
	// wide
	scatter: 320,
	"stacked-area": 320,
	bollinger: 320,
...
function getChartMinH(insight: InsightData): number {
	const type =
		((insight.chartOptions as Record<string, unknown>)
			?._auroraType as string) ?? "bar";
	...
	return (
		<div
			...
			style={cardStyle({
				...
				position: "relative",
				...style,
				height: isProgress ? "auto" : outerH,
				minHeight: isProgress ? canonicalH + 36 : undefined,
			})}>
			<div style={accentLine(accent)} />
			<p
				style={{
			...
			<div style={isProgress ? { flex: 1 } : { height: chartH, flexShrink: 0 }}>
				<AutoHeightChart insight={ins} height={chartH} preview={preview} />
			</div>
		</div>
	);
}
...
function useGridAnimation(
	...
	useEffect(() => {
		...
		let rafId: number;
		const tryAnimate = () => {
			if (!Array.from(allEls).every((el) => el.offsetWidth > 0)) {
...
function ProceduralLayout({
	...
	const baseZoom =
		insights.length <= 4 ? 1.2 : insights.length <= 7 ? 1.05 : 0.95;
	...
	return (
		<div
			...
			{gridSpec.cells.map((cell, idx) => {
				...
				const isKpi =
					!ins.chartOptions && !ins.listItems && ins.chartType !== "text";
					...
					return (
						<div
							...
							<InsightTextCard
								...
								accent={accent}
								cardRef={(el) => {
									cardRefs.current[insights.indexOf(ins)] = el;
								}}
			...
			<div
					...
					<div
						...
						{kpis.slice(0, 4).map((ins, i) => (
							<KpiCard
								key={ins.id}
								ins={ins}
								accent={ACCENT_COLORS[(i + 1) % ACCENT_COLORS.length]}
								cardRef={(el) => {
									cardRefs.current[insights.indexOf(ins)] = el;
								}}
								onClick={() => onExpand(ins)}
				...
				{secondary.map((ins, i) => (
					<ChartCard
						key={ins.id}
						ins={ins}
						accent={ACCENT_COLORS[(i + 3) % ACCENT_COLORS.length]}
						cardRef={(el) => {
							cardRefs.current[insights.indexOf(ins)] = el;
						}}
						onClick={() => onExpand(ins)}
			...
			{primary && (
				<div style={{ flex: "0 0 62%", minWidth: 0 }}>
					<ChartCard
						...
						cardRef={(el) => {
							cardRefs.current[insights.indexOf(primary)] = el;
						}}
						onClick={() => onExpand(primary)}
						style={{ height: "100%", minHeight: getChartMinH(primary) + 36 }}
					/>
				</div>
			)}
		</div>
...
function BentoAsymLayout({
	...
	const { handleNewQuery } = useGridAnimation(visible, containerRef, onReset);
	const kpis = insights
		.filter((i) => !i.chartOptions && !i.listItems)
		.slice(0, 4);
	const charts = insights.filter((i) => !!i.chartOptions);
	const primary = charts[0] ?? null;
	const secondary = charts.slice(1);
	...
	const kpiGridH =
		kpis.length > 0 ? Math.ceil(kpis.length / 2) * (KPI_H + GAP) : 0;
	const secH = secondary.reduce((s, c) => s + getChartMinH(c) + 36 + GAP, 0);
	const requiredH = HEADER_H + GAP + kpiGridH + secH + 32;
	const baseZoom =
		insights.length <= 5 ? 1.2 : insights.length <= 8 ? 1.0 : 0.85;
	const zoom = useAdaptiveZoom(containerRef, requiredH, baseZoom);
			...
			<div
				...
				{secondary.map((ins, i) => (
					<ChartCard
						key={ins.id}
						ins={ins}
						accent={ACCENT_COLORS[(i + 2) % ACCENT_COLORS.length]}
						cardRef={(el) => {
							cardRefs.current[insights.indexOf(ins)] = el;
						}}
						onClick={() => onExpand(ins)}
...
function BentoSymLayout({
	...
	const kpiGridH =
		kpisLeft.length > 0 ? Math.ceil(kpisLeft.length / 2) * (KPI_H + GAP) : 0;
	const secH = secondary.reduce((s, c) => s + getChartMinH(c) + 36 + GAP, 0);
	const requiredH = HEADER_H + GAP + kpiGridH + secH + 32;
	const baseZoom =
		insights.length <= 4
			...
			<div
				...
				{secondary.map((ins, i) => (
					<ChartCard
						key={ins.id}
						ins={ins}
						accent={ACCENT_COLORS[(i + 4) % ACCENT_COLORS.length]}
						cardRef={(el) => {
							cardRefs.current[insights.indexOf(ins)] = el;
						}}
						onClick={() => onExpand(ins)}
		...
		<div
			...
			{kpis.length > 0 && (
				<div
					style={{
						display: "grid",
						gridTemplateColumns: `repeat(${Math.max(kpis.length, 2)}, 1fr)`,
						gap: GAP,
						flexShrink: 0,
					}}>
					{kpis.map((ins, i) => (
			...
			<div
				...
				{chartA && (
					<ChartCard
						...
						accent={ACCENT_COLORS[0]}
						cardRef={(el) => {
							cardRefs.current[insights.indexOf(chartA)] = el;
						}}
						onClick={() => onExpand(chartA)}
						style={{ height: "100%" }}
					/>
				)}
				{chartB && (
					<ChartCard
						...
						cardRef={(el) => {
							cardRefs.current[insights.indexOf(chartB)] = el;
						}}
						onClick={() => onExpand(chartB)}
						style={{ height: "100%" }}
					/>
				)}
			</div>
			{rest.map((ins, i) => (
				<ChartCard
					key={ins.id}
					ins={ins}
					accent={ACCENT_COLORS[(i + 5) % ACCENT_COLORS.length]}
					cardRef={(el) => {
						cardRefs.current[insights.indexOf(ins)] = el;
					}}
					onClick={() => onExpand(ins)}
		...
		<div
			...
			{hero && (
				<div
					data-animate
					ref={(el) => {
						cardRefs.current[insights.indexOf(hero)] = el;
					}}
					onClick={() => onExpand(hero)}
					onMouseEnter={(e) =>
						hoverIn(e.currentTarget as HTMLElement, ACCENT_COLORS[0])
					}
					onMouseLeave={(e) => hoverOut(e.currentTarget as HTMLElement)}
					style={cardStyle({
						padding: "32px 40px",
...
function ExpandedModal({
	...
	const auroraData = insight.chartOptions
		? (() => {
				...
				const xAxis = opts.xAxis as { data?: string[] } | undefined;
				const series = opts.series as
					| { type?: string; data?: unknown[]; name?: string }[]
					| undefined;
				if (xAxis?.data && series?.[0]?.data) {
					return {
						labels: xAxis.data,
	...
	const modalMaxW = WIDE_TYPES.has(chartType)
		? 1280
		: TALL_TYPES.has(chartType)
			? 860
			: 900;
