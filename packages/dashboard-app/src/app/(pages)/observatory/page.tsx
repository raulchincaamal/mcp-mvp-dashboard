...
export default function ObservatoryPage() {
  ...
  // Zoom animation — runs after overlay mounts (zoomQuery set)
  useEffect(() => {
    ...
    // Reset completo antes de animar
    gsap.killTweensOf(el);
    gsap.killTweensOf(buildingRef.current);
    gsap.set(buildingRef.current, { opacity: 0, scale: 0.95, clearProps: 'none' });
  ...
  useEffect(() => {
    ...
    if (next === 'IDLE') {
      ...
      if (el && el.style.display !== 'none') {
        // Si hay error, no cerrar — el usuario debe hacer dismiss
        if (ctx.error) return;
        gsap.to(el, {
    ...
    if (next === 'REVEAL') {
      console.log('[observatory] REVEAL — insights:', ctx.insights.length);
      if (ctx.insights.length > 0) setVisibleInsights(ctx.insights);
    }

    if (next === 'PRESENTATION') {
      console.log('[observatory] PRESENTATION — overlay el:', zoomOverlayRef.current);
      const el = zoomOverlayRef.current;
      // 1. Montar presentacion PRIMERO (debajo del overlay)
      setZoomQuery(null);
      setShowPresentation(true);
      // 2. Esperar un frame para que React monte ScrollPresentation, luego fade out overlay
  ...
  const triggerZoom = useCallback((query: string, rect: DOMRect) => {
    ...
    // Pequeño delay para que React desmonte ScrollPresentation antes del zoom
  ...
  // Cuando hay error, hacer visible el overlay y mostrar el mensaje
  useEffect(() => {
    if (!ctx.error) return;
    const el = zoomOverlayRef.current;
    if (!el) return;
  ...
  const handleReset = useCallback(() => {
    ...
    if (el) {
      ...
      gsap.to(el, {
        opacity: 0, duration: 0.4, ease: 'power2.in',
        onComplete: () => gsap.set(el, { display: 'none', clearProps: 'left,top,width,height,borderRadius' }),
      });
    }
    setZoomQuery(null);
    setShowPresentation(false);
  ...
  return (
    <div style={{
      ...
      {/* Layer 4 — iOS zoom overlay: siempre montado, display:none por defecto */}
      <div
        ref={zoomOverlayRef}
        style={{
          position: 'fixed',
          ...
          {ctx.error && (
            <div style={{
              ...
              <button onClick={handleReset} style={{
                ...
                color: '#ff453a', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit',
              }}>Dismiss</button>
            </div>
          )}
        </div>
