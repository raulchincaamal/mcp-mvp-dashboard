...
const EXAMPLE_INTENTS = [
  ...
  {
    label: 'Motos por estado',
    intent: 'gráfica de ventas de motos por estado',
    icon: '🏍️',
  },
  {
    label: 'Últimas ventas',
    intent: 'tabla de las últimas 20 ventas de celulares',
    icon: '📱',
  },
  {
    label: 'Créditos atrasados',
    intent: 'estatus de créditos atrasados',
    icon: '⚠️',
  },
    ...
    async function checkLatest() {
      try {
        const res = await fetch(
          `${API_URL}/api/latest?userId=${encodeURIComponent(USER_ID)}`,
        );
        if (!res.ok) return;
    ...
    >
      ...
      <div
        style={{
          ...
          transition:
            'max-height var(--t-slow) var(--ease-in-out), padding var(--t-slow) var(--ease-in-out)',
        }}
      >
              ...
              style={{
                background:
                  loading || !intent.trim()
                    ...
                    : 'var(--primary)',
                color:
                  loading || !intent.trim() ? 'var(--text-tertiary)' : '#fff',
                border: 'none',
              ...
              onMouseEnter={(e) => {
                if (!loading && intent.trim()) {
                  ...
                  e.currentTarget.style.boxShadow =
                    '0 4px 16px rgba(73,164,216,0.4)';
                }
              }}
              onMouseLeave={(e) => {
        ...
        {credExpired && !loading && (
          ...
          >
            ...
            <div>
              ...
              >
                El token de sesión de AWS SSO ha expirado. Actualiza las
                variables en
              </p>
              <code
                style={{
          ...
          >
            <div
              style={{
                ...
                justifyContent: 'center',
                fontSize: '2.2rem',
                boxShadow: '0 0 40px rgba(73,164,216,0.15)',
              }}
            >
              ✦
            </div>
            <div>
              ...
              >
                Describe en español qué quieres visualizar y Bedrock generará el
