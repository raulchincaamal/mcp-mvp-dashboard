...
function generateRecord(id) {
  ...
  // Dynamic date distribution: spread across 2024 to today
  const today = new Date();
  const todayStr = today.toISOString().split('T')[0];
  const currentYear = today.getFullYear();

  const dateRange = faker.helpers.weightedArrayElement([
    { value: { from: '2024-01-01', to: '2024-06-30' }, weight: 15 },
    { value: { from: '2024-07-01', to: '2024-12-31' }, weight: 20 },
    { value: { from: `${currentYear - 1}-01-01`, to: `${currentYear - 1}-12-31` }, weight: 30 },
    { value: { from: `${currentYear}-01-01`, to: todayStr }, weight: 35 },
  ]);
  const fechaVenta = faker.date.between(dateRange);
  const semanasTranscurridas = Math.floor(
    (today.getTime() - fechaVenta.getTime()) /
      (7 * 24 * 60 * 60 * 1000),
  );
